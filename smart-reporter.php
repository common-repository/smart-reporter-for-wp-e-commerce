 ...
 * Plugin URI: https://www.storeapps.org/product/smart-reporter/
 ...
 * Copyright (c) 2011 - 2019 StoreApps. All rights reserved.
...
add_action( 'admin_menu', 'smart_reporter_add_admin_pages' );
function smart_reporter_add_admin_pages() {
	global $menu;

	// Add a separator for SO
	$menu[] = [ '', 'read', 'separator-smart-reporter', '', 'wp-menu-separator smart-reporter' ];

	add_menu_page( 'Smart Reporter', 'Smart Reporter','read', 'smart-reporter', 'sr_admin_page', 'dashicons-performance', 58 );
	if( defined( 'SRPRO' ) && SRPRO !== true ) {
		add_submenu_page( 'smart-reporter', __( '<span class="sr_pricing_icon"> 🔥 </span> Go Pro', 'smart-reporter-for-wp-e-commerce' ), __( '<span class="sr_pricing_icon"> 🔥 </span> Go Pro', 'smart-reporter-for-wp-e-commerce' ), 'manage_options', 'smart-reporter-pricing', 'sr_admin_page' );
	}
}

// Filters to modify SR menu position
add_filter( 'custom_menu_order', '__return_true' );
add_filter( 'menu_order', 'sr_menu_order' );
/**
 * Reorder the Smart Reporter menu items in admin.
 * Based on WC.
 *
 * @since 2.10.0
 *
 ...
 * @return array
 */
function sr_menu_order( $menu_order ) {
	// Initialize our custom order array.
	$smart_reporter_menu_order = array();

	// Get the index of our custom separator.
	$smart_reporter_separator = array_search( 'separator-smart-reporter', $menu_order, true );

	// Get index of library menu.
	$smart_reporter_admin_link = array_search( 'smart-reporter', $menu_order, true );

	// Loop through menu order and do some rearranging.
	foreach ( $menu_order as $index => $item ) {
		if ( 'smart-reporter' === $item ) {
			$smart_reporter_menu_order[] = 'separator-smart-reporter';
			$smart_reporter_menu_order[] = $item;
			$smart_reporter_menu_order[] = 'smart-reporter';

			unset( $menu_order[ $smart_reporter_separator ] );
			unset( $menu_order[ $smart_reporter_admin_link ] );
		} elseif ( ! in_array( $item, [ 'separator-smart-reporter' ], true ) ) {
			$smart_reporter_menu_order[] = $item;
		}
	}

	// Return order.
	return $smart_reporter_menu_order;
}
...
function sr_get_latest_upgrade_class() {
	...
	$latest_class = 'StoreApps_Upgrade_3_3';
	$latest_version = 0;
...
function sr_upgrade() {
	if (file_exists ( (dirname ( __FILE__ )) . '/pro/sr.js' )) {
		if ( ! class_exists( 'StoreApps_Upgrade_3_3' ) ) {
			require_once 'pro/class-storeapps-upgrade-3-3.php';
		}
...
function sr_add_support_ticket_content() {
	...
	if ( class_exists( 'StoreApps_Upgrade_3_3' ) ) {
		...
		StoreApps_Upgrade_3_3::support_ticket_content( SR_PREFIX, SR_SKU, $sr_plugin_data, $sr_license_key, SR_TEXT_DOMAIN );
	}
}
...
if ( is_admin () || ( is_multisite() && is_network_admin() ) ) {
 ...
 //    }
	...
	function sr_admin_page(){
		...
		include_once( 'pricing/class-smart-reporter-pricing.php' );
		if ( ( defined( 'SR_IS_WOO22' ) && SR_IS_WOO22 == 'false' ) && ( defined( 'SR_IS_WOO30' ) && SR_IS_WOO30 == 'false' ) ) {
			sr_console_common();
		} else if ( !empty( $_GET['page'] ) ) {
			if ( 'wc-reports' === $_GET['page'] ) {
				if ( !empty( $_GET['view'] ) && ( 'smart_reporter_old' === $_GET['view'] ) ) {
					sr_console_common();
				} else {
					sr_beta_show_console();
				}
			} else if( ( 'smart-reporter-pricing' === $_GET['page'] ) && ( class_exists( 'Smart_Reporter_Pricing' ) && is_callable( array( 'Smart_Reporter_Pricing', 'sr_show_pricing' ) ) ) ) {
				Smart_Reporter_Pricing::sr_show_pricing();
			} else if ( 'smart-reporter' === $_GET['page'] ) {
				wp_redirect( admin_url( 'admin.php?page=wc-reports&tab=smart_reporter' ) );
			}
		}
	}
				...
				?>
						<p class="wrap" style="font-size: 12px">
							<span id='sr_nav_links' style="float: right;margin-right: -1.2em;"> <?php
								...
								$check_pro_version = '';
								if ( SRPRO != true ) {
									$check_pro_version = '<span class="sr_pricing_icon"> 🔥 </span><a target="_blank" href="'. admin_url('admin.php?page=smart-reporter-pricing') .'" title="'. __( 'Explore Pro version', SR_TEXT_DOMAIN ) .'"> ' . __( 'Explore Pro version', SR_TEXT_DOMAIN ) .'</a> | ';
								}
								...
								printf ( __ ( '%1s%2s%3s%4s'), $check_pro_version, $switch_version, $before_plug_page, $after_plug_page);		
							?>
							</span>
							<?php
								if ( !empty($_GET['page']) && $_GET['page'] != 'wc-reports') {
